...
change() {
  ...
  if [ "$1" -eq 0 ]; then
    ...
    vol_text=$(( $(pactl get-sink-volume $sink | awk 'NR==1 {printf int($5)}') + $1 ))
...
sinks() {
  pid=`ps aux | grep 'st -t statusutil_sinks' | grep -v grep | awk '{print $2}'`
  mx=`xdotool getmouselocation --shell | grep X= | sed 's/X=//'`
  my=`xdotool getmouselocation --shell | grep Y= | sed 's/Y=//'`
  dunstctl close 9527
  [ -n "$pid" ] && kill $pid || st -t statusutil_sinks -g 50x15+$((mx))+$((my + 20)) -c float -e "$sink_sh" once
}
